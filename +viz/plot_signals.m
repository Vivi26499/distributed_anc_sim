function plot_signals(time, d_ch, e_ch, mic_id, Fs)
% PLOT_SIGNALS 绘制单个通道的期望信号和误差信号。
%
%   plot_signals(time, d_ch, e_ch, mic_id, Fs)
%
%   输入:
%       time    - 时间向量
%       d_ch    - 通道的期望信号
...
figure('Name', sprintf('Microphone %d Analysis', mic_id));

% --- 1. 时域信号 ---
subplot(2, 1, 1);
hold on;
plot(time, d_ch, '-', 'Color', [0.3010 0.7450 0.9330], 'LineWidth', 1.2, ...
    'DisplayName', sprintf('d_{%d}(n)', mic_id));
plot(time, e_ch, '-',  'Color', [0.8500 0.3250 0.0980], 'LineWidth', 1.2, ...
    'DisplayName', sprintf('e_{%d}(n)', mic_id));
grid on;
title(sprintf('麦克风 %d: 时域信号', mic_id));
...
plot(f, 10*log10(P_e), '-', 'Color', [0.8500 0.3250 0.0980], 'LineWidth', 1.2, ...
    'DisplayName', sprintf('PSD of e_{%d}', mic_id));
grid on;
title(sprintf('麦克风 %d: 功率谱密度 (信号末段)', mic_id));
