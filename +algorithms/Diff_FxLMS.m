function results = Diff_FxLMS(params)
    ...
    for n = 1:nSamples
        ...
        for keyNode = keys(network.Nodes)'
            node = network.Nodes(keyNode);
            node.Psi = node.W - node.StepSize * e(n, keyErrMics == node.ErrMicId) * node.xf_taps;
        end
        % 3.4.3 更新W
        for keyNode = keys(network.Nodes)'
            node = network.Nodes(keyNode);
            ...
            for neighborId = node.NeighborIds
                ...
                node.W = node.W + neighbor.Psi / numel(node.NeighborIds);
            end            
        end
    end
